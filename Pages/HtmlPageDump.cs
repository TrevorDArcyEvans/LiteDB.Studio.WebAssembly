...
public sealed class HtmlPageDump
{
  ...
  private void SpanCollectionPage()
  {
    ...
    for (var i = 0; i < 5; i++)
    {
      SpanPageID(PAGE_HEADER_SIZE + (i * 4), "DataPageList #" + i, false, true);
    }
  ...
  private int SpanPageID(int index, string caption, bool pageAddress, bool pageList)
  {
    ...
    _items[index].Href = pageID == uint.MaxValue || index == 0 ? null : "/" + (pageList ? "list/" : "") + pageID + (pageAddress ? "#" + _buffer[index + 4] : "");
